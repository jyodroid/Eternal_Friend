...
public class PermissionsHelper {
    ...
    @StringDef({LOCATION_PERMISSION, WRITE_EXTERNAL_STORAGE_PERMISSION})
    ...
    /**
     * verify is the checkbox "Don't ask again" on permission promp is checked
     *
    ...
    /**
     * Verifies the permissions are granted or else asks for missing permission.
     *
    ...
    int obtainPermissionType(@NonNull Activity activity) {
        ...
        if (!verifyPermission(activity, LOCATION_PERMISSION) &&
                !verifyPermission(activity, WRITE_EXTERNAL_STORAGE_PERMISSION)) {
            return MY_PERMISSIONS_MULTIPLE;
        } else
            //Verify if only location permission is needed
            if (!verifyPermission(activity, LOCATION_PERMISSION)) {
                return MY_PERMISSIONS_REQUEST_LOCATION_PERMISSION;
            } else
    ...
    /**
     * Request necessary permissions for functionality from an {@link Activity}
     *
        ...
        switch (permissionType) {
            case MY_PERMISSIONS_MULTIPLE:
                ...
                String[] multiplePermissionList = {
                        LOCATION_PERMISSION, WRITE_EXTERNAL_STORAGE_PERMISSION};
                //Verify if you should explain why you need permission
                if (shouldAskForPermission(
                        activity, WRITE_EXTERNAL_STORAGE_PERMISSION) ||
                        shouldAskForPermission(activity, LOCATION_PERMISSION)) {
            ...
            case MY_PERMISSIONS_REQUEST_LOCATION_PERMISSION:
                ...
                String[] cameraPermissionList = {LOCATION_PERMISSION};

                //Verify if you should explain why you need camera permission
                if (shouldAskForPermission(activity, LOCATION_PERMISSION)) {

                    showPermissionMessage(
                            "We will need permission to know your location!!",
                            okButtonText,
                            negativeButtonText,
                ...
                //don't need to explain ask for camera permission directly
                    ...
                    requestPermissions(
                            cameraPermissionList, MY_PERMISSIONS_REQUEST_LOCATION_PERMISSION, activity);
                }
                break;
            case MY_PERMISSIONS_REQUEST_WRITE_EXTERNAL_STORAGE_PERMISSION:
                ...
                if (shouldAskForPermission(activity, WRITE_EXTERNAL_STORAGE_PERMISSION)) {
                    ...
                    showPermissionMessage(
                            "We need to storage the pet's image!!",
                            okButtonText,
                            negativeButtonText,
                            writePermissionList,
                            activity,
    ...
    public static void showPermissionMessage(
        ...
        builder.setCancelable(false)
                .setTitle(activity.getString(R.string.permission_dialog_tittle))
    ...
    public static class HandleMessage extends Handler {
        ...
        public void isAskingForPermission(boolean isAskingForPermission){
            this.isAskingForPermission = isAskingForPermission;
        }
    }
}
